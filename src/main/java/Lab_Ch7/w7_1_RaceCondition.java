...
class Buffer1 //extends Semaphore
{
	...
	System.out.println(Thread.currentThread().getName() + "   " + sum + " " + Thread.currentThread().getState());
	//release(); //คำสั่งให้หลุด signal ออกจาก key --> ถ้าไม่ใส่หลังที acquire เกิด Deadlock
    }
};
